...
- Linux (internal workplace hosts, VPS hosts, personal workstations and servers)
- Linux-like (WSL and Git/Bash on work laptop)
- Windows (work laptop, kids' desktops)
- OS X (spouse's laptop)

## ToDo

- [ ] Add more documentation!
- [ ] Add scripts that do initial installation of all necessary tools. Ansible/Winget

## How to use

I'm using [chezmoi](https://www.chezmoi.io/) to manage my dotfiles: it has a couple of features that let me handle the complexity of multi-host multi-OS dotfiles much better, namely templates and ability to use Github repos as external config sources.
...
Aside from being an awesome cross-platform terminal emulator it also supports multiplexing, thus potentially replacing `tmux`.
...
Neovim is available for every platform and can be easily installed if missing.
...
This can be partially achieved with terminal emulator features (windows/panes and remote sessions), but the persistence bit is seriously awesome: this lets me not lose work if I get disconnected from the remote system.
...
NOTE: The only semi-hard requirement is that these should be cross-platform, i.e. work under Linux, OS X and Windows.
...
- [bottom](https://bottom.pages.dev/stable/)
